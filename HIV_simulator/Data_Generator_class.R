...
Data_Generator <- setRefClass(
  ...
  methods = list(
    join_datasets =
      function (Oname = 'Training_Data/Joined_TrainingSet_Kupperman', prefix =
                  '') {
...
# Data_Generator 2 --------------------------
#A variant for generating samples in the reverse order 
#TODO update s reverse field from above
Data_Generator2 <- setRefClass(
  "Master_Params_Reverse",
  ...
  methods = list(
    ...
    logspace_mu = function() {
      if (mu_number > 1) {
        mu <-
          10 ** (seq(mu_small, mu_large, by = (mu_large - mu_small) / (mu_number -
                                                                         1)))
      }
      else{
        mu <- 10 ** (mu_small)
    ...
    logspace_s = function() {
      ...
      if (s_number > 1) {
        s <-
          (10 ** (seq(
            s_small, s_large, by = (s_large - s_small) / (s_number - 1)
          ))) / N
        s_mod_number <<- s_number
        if (include_s_zero) {
          s <- append(0, s[s != 0])
          s_mod_number <<- s_number + 1
        }
      }
      else{
        s <- 10 ** (s_small)
